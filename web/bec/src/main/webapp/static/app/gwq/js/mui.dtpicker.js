...
(function($) {
	...
	var DtPicker = $.DtPicker = $.Class.extend({
		...
		_createMonth: function(current) {
			...
			if (options.customData.m) {
				...
				beginMonth=options.beginMonth==undefined?1:options.beginMonth;
				endMonth=options.endMonth ==undefined?12:options.endMonth;
				for (var m = parseInt(beginMonth); m <= parseInt(endMonth); m++) {
		...
		_createMonthPlus: function(self) {
			//var self = this;
			var options = self.options;
			var ui = self.ui;
			//生成月列表
			...
			if (options.customData.m) {
				...
				endMonth=options.endMonth ==undefined?12:options.endMonth;
				beginMonth=_getMonth(self);
				endMonth=12;
				//alert(beginMonth+"-"+endMonth)
				for (var m = parseInt(beginMonth); m <= parseInt(endMonth); m++) {
					var val = self._fill(m);
					mArray.push({
						text: val,
		...
		_createDay: function(current) {
			...
			if (options.customData.d) {
				...
				beginMonth=options.beginMonth;
				endMonth=options.endMonth;
				beginDay=options.beginDay==undefined?1:options.beginDay;
				endDay=options.endDay ==undefined?maxDay:options.endDay;
				for (var d = parseInt(beginDay); d <= parseInt(endDay); d++) {
		...
		_createHours: function(current) {
			...
			if (options.customData.h) {
				...
				beginDay=options.beginDay;
				endDay=options.endDay;
				beginHour=options.beginHour==undefined?0:options.beginHour;
				endHour=options.endHour ==undefined?23:options.endHour;
				for (var h = parseInt(beginHour); h <= parseInt(endHour); h++) {
		...
		_createHoursPlus: function(self) {
			//var self = this;
			var options = self.options;
			var ui = self.ui;
			//生成时列表
		...
		_createMinutes: function(current) {
			...
			if (options.customData.i) {
				...
				beginMinute=options.beginMinute==undefined?0:options.beginMinute;
				endinute=options.endinute ==undefined?59:options.endinute;
				for (var i = parseInt(beginMinute); i <= parseInt(endinute); i++) {
		...
		_createMinutesPlus: function(self) {
			//var self = this;
			var options = self.options;
			var ui = self.ui;
			//生成分列表
	...
	function _getMonth(self){
		var options = self.options;
		beginMonth=options.beginMonth;
		beginYear=options.beginYear;
		if(beginYear!=undefined){
			var regEx = new RegExp("\\-","gi");
			var sel=self.getSelected().value;
			date = new Date(sel.replace(regEx,"/"));
			if(parseInt(beginYear)<date.getFullYear()){
				return 1;
			}else{
	...
	function _getDate(self){
		...
		if(beginYear!=undefined){
			...
			if(parseInt(beginYear)<date.getFullYear()){
				...
				if(beginMonth!=undefined){
					if(parseInt(beginMonth)<parseInt(date.getMonth()+1)){
						return 1;
					}else{
			...
			if(parseInt(beginYear)<date.getFullYear()){
				...
				if(beginMonth!=undefined){
					if(parseInt(beginMonth)<parseInt(date.getMonth()+1)){
						...
						if(beginDay!=undefined){
							if(parseInt(beginDay)<parseInt(date.getDate())){
								return 0;
							}else{
								if(beginHour!=undefined){
									if(parseInt(beginHour)<parseInt(date.getHours())){
										return 0;
									}else{
										return beginHour;
									}
	...
	function _getMinutes(self){
		...
		beginDay=options.beginDay;
		beginHour=options.beginHour;
		beginMinute=options.beginMinute;
		if(beginYear!=undefined){
			var regEx = new RegExp("\\-","gi");
			var sel=self.getSelected().value;
			date = new Date(sel.replace(regEx,"/"));
							...
							if(parseInt(beginDay)<parseInt(date.getDate())){
								...
								if(beginHour!=undefined){
									if(parseInt(beginHour)<parseInt(date.getHours())){
										...
										if(beginMinute!=undefined){
											if(parseInt(beginMinute)<parseInt(date.getMinutes())){
												return 0;
											}else{
												return beginMinute;
											}
