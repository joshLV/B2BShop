(function($, window) {
	...
	var template = '<div id="{{id}}" class="mui-slider mui-preview-image mui-fullscreen"><div class="mui-preview-header">{{header}}<span class="mui-p-return">返回</span></div><div class="mui-slider-group"></div><div class="mui-preview-footer mui-hidden">{{footer}}</div><div class="mui-preview-loading"><span class="mui-spinner mui-spinner-white"></span></div></div>';
	var itemTemplate = '<div class="mui-slider-item mui-zoom-wrapper {{className}}"><div class="mui-zoom-scroller img-top"><img src="{{src}}" data-preview-lazyload="{{lazyload}}" style="{{style}}"  class="mui-zoom">' +
		'<p class="prev-tit">{{title}}</p>' +
		'<p class="prev-price"><span class="nologin"><span class="left">商城价：<em>会员可见</em></span><span class="right">会员价：<em>会员可见</em></span></span> <span class="login" style="display: none;"><span class="left">商城价：<em>{{scj}}</em></span><span class="right">会员价：<em>{{hyj}}</em></span></span></p>' +
		'<nav class="mui-bar mui-bar-tab" style="position: absolute;z-index: 30">' +
			'<table border="0" width="100%">' +
			'<tbody><tr>' +
			'<td>' +
			'<button class="mui-btn mui-btn-primary mui-pull-right mui- gwc" style="float: right;" goodid="{{goodid}}">' +
			'<a href="javascript:void(0);" class="ligm">立即购买</a>' +
			'</button>' +
			'<button class="mui-btn mui-btn-primary mui-pull-right mui-gwc" style="float: right;" goodid="{{goodid}}">' +
			'<a href="javascript:void(0);" class="tjgz" >关注商品</a>' +
			'</button>' +
			'</td>' +
			'</tr>' +
			'</tbody></table>' +
	...
	proto.init = function() {
		...
		this.closees = this.element.querySelector($.classSelector('.p-return'));
		if (options.footer) {
	...
	proto.initEvent = function() {
		...
		var laterCloseEvent = function() {
			!laterClose && (laterClose = $.later(function() {
				...
				self.closees.removeEventListener('tap', laterCloseEvent);
				//self.closees.removeEventListener('tap', laterCloseEvent);
		...
		this.element.addEventListener('webkitAnimationEnd', function() {
			if (self.element.classList.contains($.className('preview-out'))) { //close
				...
				self.closees.addEventListener('tap', laterCloseEvent);
				//self.closees.addEventListener('tap', laterCloseEvent);
	...
	proto.addImage = function(img) {
		...
		var goodid = img.getAttribute('goodid');
		var scj = img.getAttribute('scj');
		var hyj = img.getAttribute('hyj');
	...
	proto.refresh = function(index, groupArray) {
		...
		for (var i = 0; from < to; from++, i++) {
			...
			itemStr = itemTemplate.replace('{{src}}', itemData.src).replace('{{lazyload}}', itemData.lazyload).replace('{{style}}', style).replace('{{title}}', itemData.title).replace('{{goodid}}', itemData.goodid).replace('{{goodid}}', itemData.goodid).replace('{{scj}}', itemData.scj).replace('{{hyj}}', itemData.hyj);
		...
		$(".ligm").each(function(index,obj){
			jQuery(obj).parent().bind('tap', function(event) {
				jumpd(jQuery(obj).parent().attr("goodid"));
			})
		})
		$(".tjgz").each(function(index,obj){
			jQuery(obj).parent().bind('tap', function(event) {
				addGoodsAttention(jQuery(obj).parent().attr("goodid"));
			})
		})
