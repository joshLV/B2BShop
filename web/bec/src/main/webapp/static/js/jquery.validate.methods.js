...
jQuery.validator.addMethod("remote", function(value, element, param) {
	...
	if ( previous.old !== value ) {
		...
		var data = {};
		/*------加入url中?后面的参数的处理 2013-10-31 ------*/
		var _params = param.url.replace(/^.*?[?](.+?)(?:#.+)?$/, "$1");
        $.each(_params.split(/[&;]/), function() {
            var key = (this.split('=')[0] || "");
            var val = (this.split('=')[1] || "");
            data[key] = val;
        });
        if(param.url.indexOf('?')!=-1){
        	param = param.url.substring(0,param.url.indexOf('?'));//除去?后面的参数
        }
		/*------加入url中?后面的参数的处理 2013-10-31 END------*/
...
/*********************jquery-1.5.2.min.js的Bug修复 20140106 ***********************/
//扩展remote验证，进行POST提交，避免中文乱码的问题。
jQuery.validator.addMethod("remote", function(value, element, param) {
	if ( this.optional(element) )
		return "dependency-mismatch";
	...
	if ( previous.old !== value ) {
		...
		$.ajax($.extend(true, {
			url: param,
			//dataType: "json",/*------20140106 去掉这个类型就能正常访问 jquery-1.5.2.min.js的Bug------*/
			type: "post",
			data: data,
			success: function(response) {
				validator.settings.messages[element.name].remote = previous.originalMessage;
...
jQuery.validator.addMethod("dateGt", function(value, element, param) {
	var date01=new Date(value.replace("-", "/").replace("-", "/"));  
    var date02=new Date($(""+param).val().replace("-", "/").replace("-", "/"));  
    return date01>date02;  
}, jQuery.validator.messages.dateGt);

//日期大于等于对比元素
jQuery.validator.addMethod("dateGe", function(value, element, param) {
	var date01=new Date(value.replace("-", "/").replace("-", "/"));  
    var date02=new Date($(""+param).val().replace("-", "/").replace("-", "/"));  
    return date01>=date02;  
}, jQuery.validator.messages.dateGe);

//字母数字下划线校验
jQuery.validator.addMethod("zmsz", function(value, element) {
	return this.optional(element) || /^dync_[\w]+$/.test(value);
}, $.validator.format("以dync_开头且只允许包含英文、数字和下划线!"));

//身份证号码验证 - 身份证为15位或者18位   15位全数字  18位前17位数字最后一位数字或字母
...
}, $.validator.format("银行卡号为16位或19位!"));

//qq号码验证 5-10为数字
jQuery.validator.addMethod("QQ", function(value, element) {
	return this.optional(element) || /(^[1-9][0-9]{4,10}$)/.test(value);
}, $.validator.format("请输入正确的QQ号!"));

//金额比较
jQuery.validator.addMethod("moneyGt", function(value, element, param) {
	var down=parseInt($(param).val());
	var up=parseInt(value);
	return up>down;
}, jQuery.validator.messages.moneyGt);

//金额比较
jQuery.validator.addMethod("moneyGG", function(value, element, param) {
	var down=parseInt($(param).val());
	var up=parseInt(value);	
	return up<down;
}, jQuery.validator.messages.moneyGG);

//单据编号验证
jQuery.validator.addMethod("ticNo", function(value, element) {
	return this.optional(element) || /^[A-Za-z0-9]*$/.test(value);
}, $.validator.format("请输入数字或字母!"));
