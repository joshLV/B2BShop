...
function searchG(key,param){
    if (key != undefined && key != "") {
        ...
        if (param.length > 0 && param != null && param != "" && param != "undefined" && param != undefined && param != "null") {
            window.location.href = base + "search/s/" + encodeURI(encodeURI($.trim(key).replace(/\//g, '址达司杠').replace(/\./g, '址达司点号'))) + "?" + param;
        } else {
            window.location.href = base + "search/s/" + encodeURI(encodeURI($.trim(key).replace(/\//g, '址达司杠').replace(/\./g, '址达司点号')));
        }
    }
}
