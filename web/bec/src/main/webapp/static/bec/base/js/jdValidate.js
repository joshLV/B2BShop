...
var validatePrompt = {
    ...
    reallName: {
        ...
        error: {
            ...
            fullNumberName: "<span>真实姓名,不能是纯数字，请确认输入的是真实姓名</span>"
        },
        onFocusExpand: function () {
    ...
    iDcode: {
        ...
        isNull: "请输入身份证号",
        error: {
            beUsed: "该身份证号已被使用，请重新输入。",
            badLength: "身份证号长度为18位",
            //badFormat: "用户名只能由中文、英文、数字及“_”、“-”组成",
            badFormat: "身份证号格式错误",
            fullNumberName: "<span>身份证号只能由数字，英文组成，请确认输入的是身份证号</span>"
        },
        onFocusExpand: function () {
...
var validateFunction = {
    regName: function (option) {
        ...
        if (validateRules.isNull(regName) || regName == '手机号') {
            option.element.removeClass(validateSettings.INPUT_style2).removeClass(validateSettings.INPUT_style1);
    ...
    reallName: function (option) {
        var nickName = option.value;

        if (validateRules.isNull(nickName) || nickName == '真实姓名') {
            option.element.removeClass(validateSettings.INPUT_style2).removeClass(validateSettings.INPUT_style1);
            $("#nickName_error").removeClass().empty();
            return;
        }
    ...
    iDcode: function (option) {
        var iDcode = option.value;

        if (validateRules.isNull(iDcode) || iDcode == '身份证号') {
            option.element.removeClass(validateSettings.INPUT_style2).removeClass(validateSettings.INPUT_style1);
            $("#iDcode_error").removeClass().empty();
            return;
        }
        if(!validateRules.isIdCode(iDcode)){
            validateSettings.error.run(option, option.prompts.error.badFormat);
            return;
        }

        checkUniqueIDCode(option);
...
function checkUnique(option) {
	$.ajax({
        ...
        data:{'userName':option.value,'property':'mobile'},
        datatype: 'json',
        error: function () {
        	validateSettings.error.run(option, "<span>手机号校验失败，请稍后重试。</span>");
        },
        success: function(result) {
        	if(result){
        		validateSettings.succeed.run(option);
        	}else{
        		validateSettings.error.run(option, "手机号已经被占用，请重新输入手机号。");
        	}
        }
    });
}

function checkUniqueIDCode(option) {
    $.ajax({
        type: "post",
        url: base+"member/username/check",
        data:{'userName':option.value,'property':'iDCode'},
        datatype: 'json',
        error: function () {
            validateSettings.error.run(option, "<span>身份证号校验失败，请稍后重试。</span>");
        },
        success: function(result) {
            if(result){
                validateSettings.succeed.run(option);
            }else{
                validateSettings.error.run(option, "身份证号已经被占用，请重新输入身份证号。");
            }
        }
    });
}
