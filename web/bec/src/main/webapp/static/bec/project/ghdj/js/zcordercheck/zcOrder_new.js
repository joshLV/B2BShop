...
function use_NewConsignee() {
    ...
    if(num>6){
        layer.alert("抱歉，地址信息最多可创建6条。",{icon:2});
        return;
    }
...
*/
    installReloadForm($("#zcgoodsId").val(),deliveryMethodId,paymentMethodId,deliveryTimeId,order_deliveryTimeName,receiverId,invoiceContent,invoiceTitle,createOrderType,isMemberPrice,hasInsuranceFee);
    $("#reloadForm").submit();
    return false;
}
function installReloadForm(o,deliveryMethodId,paymentMethodId,deliveryTimeId,order_deliveryTimeName,receiverId,invoiceContent,invoiceTitle,createOrderType,isMemberPrice,hasInsuranceFee){
    ...
    html += '<input type="hidden" name="deliveryMethodId" id="rf_deliveryMethodId" value="' + deliveryMethodId + '" >';//配送Id
    html += '<input type="hidden" name="paymentMethodId" id="rf_paymentMethodId" value="' + paymentMethodId + '" >';//支付方式
    html += '<input type="hidden" name="deliveryTimeId" id="rf_deliveryTimeId" value="' + deliveryTimeId + '" >';//配送时间
    html += '<input type="hidden" name="order_deliveryTimeName" id="rf_order_deliveryTimeName" value="' + order_deliveryTimeName + '" >'; //配送名称
    html += '<input type="hidden" name="receiverId" id="rf_receiverId" value="' + receiverId + '" >';//收货地址Id
    ...
    html += '<input type="hidden" name="isMemberPrice" id="rf_isMemberPrice" value="' + isMemberPrice + '" >';//会员价
    html += '<input type="hidden" name="hasInsuranceFee" id="rf_hasInsuranceFee" value="' + hasInsuranceFee + '" >';//保价
    html += '<input type="hidden" name="dealNum" id="dealNum" value="' + $("#dealNum").val() + '" >';//众筹商品购买数量
