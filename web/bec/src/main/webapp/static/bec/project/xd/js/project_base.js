            ...
            if(data.status=='success'){
                ...
                $.each(json,function(i,n){
                    ...
                    data= data+'<ul id="mcart-sigle">'+
                        ...
                        '<img src="'+(json[i].pic==undefined?'':json[i].pic)+'" width="50" height="50" alt="" class="err-product"></a>'+
                        '</div>'+
                        '<div class="p-name f-fl">'+
                        '<a href="'+base+'item/'+json[i].goodsId+'.html" target="_blank">'+json[i].name+'</a>'+
                        '</div>'+
                        '<div class="p-detail f-fr ar">'+
                            //'<span class="p-price">单价：￥'+json[i].price+'<br>数量：'+json[i].quantity+'</span><br><a href="javascript:del(\''+json[i].id+'\');">删除</a>'+
                        '<span class="p-price">单价：￥'+showBatch+'<br>数量：'+json[i].quantity+'</span><br><a href="javascript:delCart(\''+json[i].id+'\');">删除</a>'+
                        '</div>'+
                        '</li>'+
                        '</ul>'+
                        '<ul id="mcart-gift"></ul>';
                ...
                divtotal.text("");// 清空数据
                var data1='共<b>'+totalshop+'</b>件商品　共计<strong>￥ '+totalmoney.toFixed(2)+'</strong><br>'+
