...
function loadGoods(){
    ...
    if( !flag ){//加载完了，不需要再加载
        ...
        $.ajax({
            type: "post",
            url: $("#basePath").val() + "/newArrivals/ajaxList",
            data: {
                mid: $("#cur_mid").val(),
                mn:$("#mn").val(),
                sn:$("#sn").val(),
            ...
            success: function(data) {
                ...
                if (data.status == 'success') {
                    ...
                    if( data.priceHide == 'hide' ){
                        ...
                        $.each(data.goodslist, function (goods_index, goods) {
                            ...
                            html += '<td width="300" class="textoverflow">';
                                html += '<a href="' + $("#basePath").val() + '/item/' + goods.id + '.html" target="_blank" title="'+goods.name+'">';
                                html += goods.name;
                                html += '</a>';
                            html += '</td>';

                            html += '<td width="140" style="text-align:center">商品规格</td>';
                            html += '<td width="220" style="text-align:center">会员价格</td>';
                            html += '<td width="210" style="text-align:center">当前价格</td>';
                            html += '<td width="144" style="text-align:center">数    量</td>';
                            /*20160505 dzz 隐藏库存显示*/
                            ...
                            if( typeof(goods.pic) != 'undefined' && goods.pic != '' ){
                                html += goods.pic  +'@90w_90h.png';
                            }
                            html += '" width="90" height="90" alt="'+goods.name+'" title="'+goods.name+'" style="border: 1px solid #E6E6E6;">';
                            html += '</a>';
                            html += '</td>';

                            html += '<td width="300"><div class="pro_name">';
                            html += '<p><em>所属商家：</em><a href="javascript:void(0);" onclick="allG(\'' + goods.shopId + '\',\'' + goods.shopName + '\',\'\',\'\',\'vt=list\')" >' + goods.shopName + '</a></p>';
                            html += '<p class="textoverflow"><em>商品简称：</em><a href="javascript:void(0);" onclick="allG(\'' + $("#cur_mid").val() + '\',\'\',\'' + goods.shortName + '\',\'\',\'vt=list\')">' + goods.shortName + '</a></p>';
                            ...
                            html += '<td width="140" class="pro_color" >';
                            ...
                            html += '<li>';
                            html += '<i><img src="'+$("#basePath").val()+'/static/bec/project/ghj/img/sc.png" alt="商城价" title="商城价"></i>';
                            if (goods.enableBatchPrice == '1') {
                            ...
                            html += '</li>';
                            html += '<li class="red">';
                            html += '<i><img src="'+$("#basePath").val()+'/static/bec/project/ghj/img/hy.png" alt="会员价" title="会员价"></i>';

                            if (goods.enableBatchPrice == '1') {
                                html += '￥' + goods.batchPrice;
                            } else {
                                html += '￥' + (parseFloat(goods.price)*(1-parseFloat(goods.defaultPrice)/100)).toFixed(2);
                            }
                            ...
                            html += '</li>';
                            html += '<li class="red">';
                            html += '<i><img src="'+$("#basePath").val()+'/static/bec/project/ghj/img/hy.png" alt="商城价" title="商城价"></i>';
                            html += '<span id="'+goods.id+'_nowMemPrice">';
                            if (goods.batchPrice != '' && goods.batchPrice != "0.00") {
                                html += '￥' + (parseFloat(goods.maxBatchPrice)*(1 - parseFloat(goods.defaultPrice)/100)).toFixed(2);
                            } else {
                                html += '￥' + (parseFloat(goods.price)*(1 - parseFloat(goods.defaultPrice)/100)).toFixed(2);
                            }
                            html += '</span>';
                            ...
                            if (0 < goods.stock) {
                                ...
                                html += '       class="shop_add" readonly="true" onfocus="this.blur()">';
                                html += '<input type="text" name="textfield2" id="nowGoodsNum_' + goods.id + '"';
                                html += '       class="shop_number" placeholder="1" readonly="true"';
                            ...
                            html += '共&nbsp;&nbsp;';
                            html += '<span id="unitNum_' + goods.id + '" style="font-weight: bold;color: red;">';
                            if (goods.enableBatchPrice == '1') {
                                html += goods.startBatchNum / goods.packageModulus;
                            } else {
                                html += goods.startNum / goods.packageModulus;
                            }
                            html += '</span>&nbsp;&nbsp;';
                            if (typeof(goods.packageUnit) != 'undefined' && goods.packageUnit != '') {
                                html += goods.packageUnit;
                ...
                //重新绑定复选框
                $('#cartListForm input:checkbox').iCheck({
