            ...
            <li>
                ...
                <div class="info">
                    <p style="position: relative;"><a  href="${base}/item/whichapp/${gl.id}.html" style="width: calc(100% - 31px);"> ${gl.name}</a>
                        <!--<a href="#" style="padding-right: 0 !important;" class="add-gwc"><img src="${path}/images/add_gwc.png" style="width: 25px;  display: inline;"></a>-->
                    </p>
...
function loadData(){
    ...
    var url = "${base}/list/getGoodsList";
    var data={
        cat:$("#catelogPath").val(),
        price:$("#priceFlag").val(),
        brand:$("#brandFlag").val(),
    ...
    //var url ="${base}/list/getGoodsList?whichclient=whichapp&catelogPath=${grandCatelog.id}<#if parentCatelog??>,${parentCatelog.id}</#if><#if catelog??>,${catelog.id}</#if><#if pager.orderBy??>&orderBy=${pager.orderBy}</#if>&pageNumber="+pageNumber;
    jQuery.ajax({
        type: "post",
        url: url,
        data: data,
