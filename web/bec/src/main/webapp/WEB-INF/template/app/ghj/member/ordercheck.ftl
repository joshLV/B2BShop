    ...
    <div class="mui-sh-box">
        ...
        <div >
            ...
            <div id="payMent" style="border-bottom: 1px solid #d8d8d8; margin-bottom: 7px; padding-bottom: 2px;">
                ...
                <p><div class="delivery-selected" style="font-size: 14px;">${deliveryMethod.name}</div>
            ...
            <div class="hide" id="payMentList">
                <div id="payment-ship" style="position: static;">
                    ...
                    <div id="shipment" style="padding-top:0px;">
                        ...
                        <div class="item" style="height: 28px;" id="psfs_${delivery.id}">
                            <div class="label">
                        ...
                        <div class="order-goods" style="padding-bottom: 15px;">
                            ...
                            <div class="info"> <a href="${base}/item/whichapp/${cartItem.goodsId}.html">${cartItem.name}</a>
                               ...
                               <h4 id="${cartItem.goodsId}">¥${cartItem.price}<span style="padding-left: 10px; font-size: 13px; color: #999; font-weight: normal; ">等级优惠：${(cartItem.price?number * memberRankScale?number)?string('currency')}</span></h4>
                                <div><p>共${cartItem.quantity}<#if cartItem.unit??>${cartItem.unit}<#else>件</#if></p></div>
                            </div>
                        </div>
...
<script>
    ...
    var __areaPath="${receiver.areaPath}";
    if(__areaPath!=""){
        __areaPath=__areaPath.split(",");
        if(__areaPath[0]!='610000'){
            $("#psfs_1605111012254171155").hide();
        }else{
            if(__areaPath[1]!='610100'){
                $("#psfs_1605111012254171155").hide();
            }
        }
    }
...
//                    }
	...
	if(invoiceContent=="no"){
	    invoiceTitle="";
	    invoiceType="";
	}
        installReloadFormDate($("#cartItemIds").val(),deliveryMethodId,paymentMethodId,deliveryTimeId,order_deliveryTimeName,receiverId,invoiceContent,invoiceTitle,createOrderType,isMemberPrice,hasInsuranceFee,invoiceType);
    ...
    function installReloadFormDate(o,deliveryMethodId,paymentMethodId,deliveryTimeId,order_deliveryTimeName,receiverId,invoiceContent,invoiceTitle,createOrderType,isMemberPrice,hasInsuranceFee,invoiceType){
        var html = '';
	...
	html += '<input type="hidden" name="invoiceType" id="rf_invoiceType" value="' + invoiceType + '" >';
    ...
    function save_Invoice() {
        ...
        if(jQuery("#invoiceContent").val()=="no") {
            ...
            if(jQuery(".selected[type='invoiceType']").text()==""){
			    ...
			    //jQuery("#invoiceTitle").val(jQuery(".selected[type='invoiceTitle'] a").val());
			    //jQuery("#invoiceContent").val(jQuery(".selected[type='invoiceContent']").val());
			    //jQuery("#invoiceType").val(jQuery(".selected[type='invoiceType']").val());
