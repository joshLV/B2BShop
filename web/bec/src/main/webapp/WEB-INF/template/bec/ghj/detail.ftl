    ...
    <div class="pro_bg f-cb">
            ...
            <div class="pro_info_r">
                ...
                <div class="pfj" id="staticBId">
                    <div class="pfj_t">
                        ...
                        <em class="f-fs2" id="staticB2Id"  style="font-size: 16px;display: none;">￥${goods.price?string("##0.00")}&nbsp;</em>
                    </div>
                    <div class="pfj_t">
                        <span>会&nbsp;&nbsp;员&nbsp;价：</span>
                        <em class="gray f-fs2 f-line" id="staticB3Id">会员可见!</em>
                        <em class="gray f-fs2 f-line red" id="staticB4Id" style="font-size: 16px;display: none;"> ￥${(goods.price * (1 - goods.defaultPrice/100))?string("##0.00")}&nbsp;</em>
                    </div>
                </div>
                ...
                <dl id="shopNameService">
                    <dt>服&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;务：</dt>
                    <dd>由 <#if shop.name??><#--<a href='${base}/shop/${shop.urlAddress}' class="ls shop-ghs">帮商乐${shop.name}</a>--><a class="ls shop-ghs">帮商乐</a><#else ><em style="float: none;">本商城</em></#if>  发货并提供售后服务
                        <#if shop.qqKeFu??>
                        ...
                        <#if shop.deliveryDay??>，<font style="color: #e12f45;">${shop.deliveryDay}</font></#if>
                    </dd>
                </dl>
                ...
                <dl>
                    ...
                    <div style="float: left; width: 380px;">
                        ...
                        <div style="display: none;" class="list-more" id="more_btn" <#if brandMore == 'true' >style="display: none; "</#if> onclick="$('#less_btn').show();$('#more_btn').hide();$('#brand_more').show();">
                            <a href="javascript:void(0)">更多</a><i></i>
                        </div>
                        <div style="display: none;" class="list-less" id="less_btn" <#if brandMore != 'true' >style="display: none;"</#if>  onclick="$('#less_btn').hide();$('#more_btn').show();$('#brand_more').hide();">
                ...
                <dl>
                    ...
                    <ul  id="choose" style="float: left;">
                        <div style="float: left; width: 455px;">
                            <div style="float: left; display: none;" class="dt">
                                其它：
                            </div>
                            <div class="list-more" id="sp_more_btn" <#if spInfoMap["goodsSpList"]?size <= 2 >style="display: none; "</#if> onclick="$('#sp_less_btn').show();$('#sp_more_btn').hide();$('.sp_li').show();">
                                <a href="javascript:void(0)">更多</a><i></i>
                            </div>
                            <div class="list-less" id="sp_less_btn" style="display: none;"  onclick="$('#sp_less_btn').hide();$('#sp_more_btn').show();$('.sp_li').hide();">
                                <a href="javascript:void(0)">收起</a><i></i>
                            </div>
                        </div>
                        <div style="float: left;">
                        <#list spInfoMap["goodsSpList"] as sp>
                            <li id="choose-${sp.id}" <#if sp_index &gt; 2> class="sp_li"  style="display: none;"</#if>>
                                <div class="dt" id="${sp.id}_sp">${sp.name}：</div>
                                <div style="/*background: #000;*/ float: left; width: 380px;">
                                <#assign i = 0>
                    ...
                    <dd>
                        ...
                        <div class="wrap-input f-cb" style="width: 300px; height: 32px; line-height: 32px;">
                            ...
                            <#if goods.stock <= 0>
                                ...
                                <!-- 20160505 dzz 取消库存显示 -->
                                <#--<div style="color: red;font-size: 14px;font-weight: bold; margin: 0 8px 0 5px; float: left;">[无货]</div>-->
                            <#else>
                                <a href="javascript:void(0);" onclick="reduce();" class="numbtn" title="减少数量">－</a>
                                <input type="text" class="text" id="buy-num" style="float: left"
                                       value="<#if goods.enableBatchPrice == '1'>${goods.startBatchNum}<#else>${goods.startNum}</#if>"
                                       onkeyup="if(this.value.length==0){this.value=this.value.replace(/[^0-9]/g,'');disableBtn();}else{this.value=this.value.replace(/\D/g,'');chengeBatchPriceByBuyNum();disableBtn();}"
                                       onafterpaste="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'';chengeBatchPriceByBuyNum();disableBtn();)}else{this.value=this.value.replace(/\D/g,'');chengeBatchPriceByBuyNum();disableBtn();}"
                                       onchange="changeNum()"/>
                            ...
                            <!-- 20160505 dzz 取消库存显示 -->
                            <#--库存${goods.stock}件-->
                            <div id="calcBuyCount" style="color: red;font-size: 14px;font-weight: bold; margin: 0 8px 0 5px; float: left;">
                                共&nbsp;
                                <span id="calcBuyCountSpan">1</span>
                                &nbsp;<#if goods.packageUnit??>${goods.packageUnit}<#else>件</#if>
                            </div>
                        </div>
                ...
                <dl class="gm">
                    ...
                    <#if goods.isMarketable != 0 && goods.isDelete != '0'>
                        <#--<dd id="buyHideId">
                            <a id="goBuyUrl" href="javascript:login();" class="pro_gms" style="display: none;">立即购买</a>
                            <a id="InitCartUrl" href="javascript:login();" class="pro_gms pro_gwcs" style="display: none;">加入购物车</a>
                        </dd>-->
                        <#if goods.stock <= 0>
                            <dd id="stockzeroId" style="display: none;">
                                <button id="goBuyUrl"  class="pro_gms" >立即购买</button>
                                <button id="InitCartUrl"  class="pro_gms pro_gwcs">加入购物车</button>
                            </dd>
                        <#else>
                            <dd id="stocknozeroId" style="display: none;">
                                <button id="goBuyUrl" onclick="judgeAuth('${base}','addCarts',addCart.bind({},'${goods.id}',$('#buy-num').val(),'goBuy'));" class="pro_gm">立即购买</button>
                                <button id="InitCartUrl" onclick="judgeAuth('${base}','addCarts',addCart.bind({},'${goods.id}',$('#buy-num').val(),'add'));" class="pro_gm pro_gwc" >加入购物车</button>
                            </dd>
                        </#if>
                    </#if>
                </dl>
                ...
                <dl>
                <#--${goods.attentionCount}---->
                    <#if goods.attentionCount==-1>
                    ...
                    <#elseif goods.attentionCount==0>
                        <div class="attention-shop"><a onclick="judgeAuth('${base}','addAttention',addGoodsAttention.bind({},'${goods.id}',this));" href="javascript:void(0);">关注该商品</a></div>
                    <#elseif goods.attentionCount&gt;0>
                        <div class="attention-shop-on"><a href="javascript:void(0);">已关注</a></div>
                    </#if>
                </dl>
            </div>
        </div>
        ...
        <div class="pr">
            ...
            <div class="box_wwwzzjs_net">
                ...
                <div class="pr_dian">
                    <span link="#tabs1" class="on">1</span>
                    <span link="#tabs2">2</span>
                ...
                <div id="tab2" class="neir">
                    <div class="grade_bar">
                        ...
                        <div class="tip-txt f-fl">
                            <div class="gold"><a href="${base}/order/review?goodsId=${goods.id}" target="_blank" title="发评价赢金币" class="z_btn review_btn">发评价</a></div>
                            <p>您可对已购商品进行评价</p>
                        </div>
                    </div>
    ...
    function getMaxSalesCountGoods(obj){
        ...
        if(!isEmpty(id)){
            $.ajax({
                type: 'post',
                async: false,
                data: "ajaxCatelogId="+id+"&gid=${goods.id}&r="+ new Date(),
...
//                            $("#"+goodsId+"_nowPrice").text("￥"+parseInt(data.batchPrices).toFixed(2));
        ...
        disableBtn();
        calcBuyCount($("#buy-num").val(), defPackageModulus)
    }
    function add() {
        //包装系数 TianYu 2016-2-17 09:19:00
        var goodsId = $("#goodsId").val();
...
//                            $("#"+goodsId+"_nowPrice").text("￥"+parseFloat(data.batchPrices).toFixed(2));
    ...
    function changeNum(){
        ...
        if(parseInt($("#buy-num").val())>9999) {
            $("#buy-num").attr("style", "width:40px;")
        }
...
//                            $("#"+goodsId+"_nowPrice").text("￥"+parseInt(data.batchPrices).toFixed(2));
    ...
    function makeHtml(review,objHtml){
        ...
        html +='<div class="time f-fr">购买时间：&nbsp;'+review.buyDateStr+'</div>';
        html +='</div>';
        html +='<div>';
        html +='<dl><dt>评论内容：</dt><dd>'+review.content+'</dd></dl>';
        html +='<dl><dt>评论时间：</dt><dd>'+review.cdStr+'</dd></dl>';
        html +='</div>';
...
//                    $(item).removeClass("btn-coll").addClass("showattention");
                    $(item).text('已关注');
                    $(item).attr("onclick", "");
                    $($(item).parent()).attr("class", "attention-shop-on");
...
//                    $(item).removeClass("btn-coll").addClass("showattention");
     ...
     * 公式:
     * @param num
     * @param packageModulus
     */
    function calcBuyCount(num, packageModulus) {
        var  calcResult = "";
        if(num%packageModulus==0) {
            calcResult = parseInt(num/packageModulus);
        } else {
            calcResult = parseInt(num/packageModulus)+1;
        }
        $("#calcBuyCountSpan").text(calcResult);
    }
</script>
