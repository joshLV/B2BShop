...
<style>
    ...
    .shopInfo .mt h2{ line-height: 43px; height:45px; font-size: 15px; color: #444; margin-bottom:15px;background:url("${base}/static/bec/project/xd/images/dp_bg.gif") left top repeat-x; overflow: hidden;}
    ...
    .catelog .mc .threeLevel .selected{border-left: 4px solid #db0000;}
</style>
<div class="shopInfo">
	<div class="mt">
...
<div class="catelog">
	<div class="mc">
        ...
        <#list parentCatelogList as parent>
            <div class="oneLevelarea menu_list firstpane" id="firstpane_${parent.id}">
                ...
                <div class="threeLevel menu_body" style="display:none;">
                    <ul>
                        <#list childCatelogMap["${(parent.id)!}"] as child>
                            <li><a title="${(child.name)!}" id="cateloge_${child.id}" parentid="firstpane_${parent.id}" href="${base}/shop/goodsShop.html?shopId=${shop.id}&catlogId=${child.id}">${(child.name)!}</a></li>
                        </#list>
                    </ul>
                </div>
            </div>
...
<script type=text/javascript>
    ...
    <#if cateloge?? && cateloge.id??>
        $("#"+ $("#cateloge_${cateloge.id}").attr("parentid") +" .menu_body:eq(0)").show();
        $("#cateloge_${cateloge.id}").parent().addClass("selected");
    ...
    <#else>
        $(".firstpane:eq(0) .menu_body:eq(0)").show();
        $("#firstpane div.menu_head").click(function(){
            $(".menu_list .menu_body").slideUp("slow");
            $(this).addClass("current").next("div.menu_body").slideToggle(300).siblings("div.menu_body").slideUp("slow");
            $(this).siblings().removeClass("current");
