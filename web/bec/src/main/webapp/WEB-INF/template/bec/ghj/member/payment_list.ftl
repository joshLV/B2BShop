...
<div class="content" style="background: #fff; width: 1120px; margin:15px auto 15px auto;  padding: 40px; border-top:4px solid #cee3c2; ">
	<div class="orderbody">
		<div class="mc">
			...
			<div style="width: 1000px; float: left;">
				...
				<div class="pay-total">
                    <#if payAmountType != 'deposit'>总支付金额：<#else>定金金额：</#if>
                    <strong class="ftx-01" >￥${total}</strong>元<br></div>
            </div>
            <ul class="list-orderinfo">
                <li>订单号：${order.orderSn}</li>
                <li <#if payAmountType == 'deposit'>class="li-last"</#if>>订单金额：<strong class="ftx-01">￥${order.totalAmount}</strong> 元</li>

                <#if payAmountType != 'deposit'>
    ...
    <div class="banking">
        ...
        <div class="clr"></div>
		<div class="online_pay_amount" style="display: none;">
			<span class="label">支付金额：</span>
			<span class="money">
                <input class="float_input" type="text" name="onlinePayAmount" id="onlinePayAmount" value="<#if payAmountType != 'deposit'>${unpaidAmountTotal?string('##0.00')}<#else>${total?string('##0.00')}</#if>"
                       onblur="changeOnlinePayAmount( );"
                       onkeyup="this.value = floatFormat( this.value, '2', '<#if payAmountType != 'deposit'>${unpaidAmountTotal?string('##0.00')}<#else>${total?string('##0.00')}</#if>' );"
                       onafterpaste="this.value = floatFormat(this.value, '2' ,'<#if payAmountType != 'deposit'>${unpaidAmountTotal?string('##0.00')}<#else>${total?string('##0.00')}</#if>'  );"
                       >
            </span>
			<span class="unit">元</span>
		</div>
...
<script type="text/javascript" src="${base}/static/bec/project/ghj/js/payment_list.js"></script>
