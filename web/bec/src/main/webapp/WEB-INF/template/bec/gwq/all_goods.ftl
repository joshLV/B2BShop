...
<style type="text/css">
    ...
    #filter .fore1 .order .zhuanhuan b{
        background: rgba(0, 0, 0, 0) url("${base}/static/bec/project/gwq/images/wholesale.png") no-repeat scroll 0 0;
        float: right;
        height: 11px;
        margin-left: 5px;
        margin-right: 10px;
                    ...
                    <dl class="order">
                        ...
                        <dd <#if pager.orderBy=="scoreCount" && pager.orderType=="desc">class="arrow_write_bottom" <#elseif pager.orderBy=="scoreCount" && pager.orderType=="asc">class="curr" <#else>class="arrow"</#if>>
                            <#if pager.orderBy=="scoreCount" && pager.orderType=="desc">
                                <a href="javascript:void(0)"  onclick="allG('${mid}','','','orderBy=scoreCount&orderType=asc&pageNumber=${pager.pageNumber}')">评论数</a><b></b>
                            <#elseif pager.orderBy=="scoreCount" && pager.orderType=="asc">
                                <a href="javascript:void(0)"  onclick="allG('${mid}','','','orderBy=scoreCount&orderType=desc&pageNumber=${pager.pageNumber}')">评论数</a><b></b>
                            <#else>
                                <a href="javascript:void(0)"  onclick="allG('${mid}','','','orderBy=scoreCount&orderType=asc&pageNumber=${pager.pageNumber}')">评论数</a><b></b>
                            </#if>
                        </dd>
                    ...
                    <li index="0" sku="${gl.id}" selfservice="0">
                        <div class="search_content_box">
                            <div class="search_content_mainimg">
                                ...
                                <img alt="${gl.name}" data-img="1" src="${gl.image}@263w_350h.png" title="${gl.name}">
                            </div>
                            <p>
                            ...
                            <p>
                                <span class="company_names"><a target="_blank" href="${base}/item/${gl.id}.html">已有<#if gl.scoreCount != '' && gl.scoreCount &lt; 0>${gl.scoreCount}<#else>0</#if>人评价</a></span>
                            ...
                            <p class="search_box_bottom search_area_p">
                                <span class="search_area">
                                    <a stock="1010626351" tpl="1" class="search-btn-buy" onclick="addCart('${gl.id}','1');" href="javascript:void(0);"  ><i></i>加入购物车</a>
                                </span>
