...
public class BaseMallController extends BaseController {
	...
	@Resource
	private BecLogService becLogService;
	/**
	 * 登录类型
	 */
	...
	public LoginType getLoginType(HttpServletRequest request) {
		return (LoginType) request.getSession().getAttribute(Global.CURRENT_LOGIN_TYPE);
	}
	/**
	 * 获取当前登录的会员
	 * 
	 * @return
	 */
	public Buyer getCurrentMember(HttpServletRequest request) {
		return (Buyer) request.getSession().getAttribute(Global.CURRENT_MERMBER);
	}
	 ...
	 * @return
	 */
	public BuyerOperator getCurrentOperator(HttpServletRequest request) {
		return (BuyerOperator) request.getSession().getAttribute(Global.CURRENT_OPERATOR);
	}
	 ...
	 * @return
	 */
	public String getOperatorRole(HttpServletRequest request) {
		return (String) request.getSession().getAttribute(Global.CURRENT_ROLE);
	}
	 ...
	 * **/
	public void saveBecLog(HttpServletRequest request, String logType, String remark, Map<String, Object> paramMap)  {
		Buyer currentMember = getCurrentMember(request); //获取当前用户信息

		try {
			/*bec组装登录日志信息*/
			BecLog becLog = new BecLog();
			becLog.setLogIp(IpUtil.getIpAddr(request));		//IP地址信息
			becLog.setLogType(logType);	//登录类型
			becLog.setTenantId(getTenantId());//租户ID
			becLog.setUserName(StringUtil.isNotEmpty(currentMember)? currentMember.getUserName() : "异常账户");//用户名信息
			becLog.setUserNamecn(StringUtil.isNotEmpty(currentMember)? currentMember.getRealName() : "异常账户");//用户真实姓名
			becLog.setCreateDate(new Date());
			becLog.setCreateUser(StringUtil.isNotEmpty(currentMember)?currentMember.getId() : "异常账户");
			becLog.setLogBeizhu(remark);
			becLog.setBrowserType(BrowserUtils.checkBrowse(request));
			becLogService.save(getParamMap(),becLog);
		}catch (Exception e){
