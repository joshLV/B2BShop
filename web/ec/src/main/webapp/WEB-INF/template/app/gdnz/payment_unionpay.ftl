...
<div class="content"><img style="zoom:1;" src="${base}/static/app/gdnz/images/loading.gif"/></div>-->
<form id = "pay_form" action="${requestOrder.requestUrl}" method="post" style="display: none">

</form>
<form id  = "pay_formdata">
...
<script>
    layer.open({type: 2});
    jQuery("#pay_form").attr("action",jQuery("#pay_form").attr("action") + "?" + jQuery("#pay_formdata").serialize());
