...
<#include "app/gdnz/head.ftl">
    ...
    <style>
       ...
       .mui-sh-box{ width: 100%;
        ...
        .order-list .bh a{ padding-left:3px; color:#ea5313;}
        ...
        .btn{color:#ffffff; background: #ea5313; padding: 5px 15px;  -webkit-border-radius: 3px;
            -moz-border-radius: 3px;
            border-radius: 3px;}
        .btn-submit {height: 30px;margin-right: 10px; padding: 0 15px; float: left;line-height: 30px;font-size: 14px;background: #ea5313;border-radius: 3px;}
        .group {zoom: 1;}
        .form-btn {height: 30px;padding-top: 4px;}
        .owl-carousel{
...
<div class="mui-content" style="padding-bottom:50px; margin-top:14px;">
    ...
    <div class="mui-sh-box">
        <form id="orderCheckInfo">
            ...
            <!--配送方式-->
            <input type="hidden" id="deliveryMethodId" name="deliveryMethodId" value="${deliveryMethod.id}">
            <!--送货时间 Auth:hks Time:2015-11-13 14:37:52 Start -->
            <input type="hidden" id="order_deliveryMethodId" name="order_deliveryTimeId" value="${deliveryTimeList[0].id}">
            <input type="hidden" id="order_deliveryMethodName" name="order_deliveryTimeName" value="${deliveryTimeList[0].introduce}">
        ...
        <div class="step-title" style="padding-left:10px;padding-right:10px;" id="goodsListTitle">
            <span style="font-size: 16px;font-weight: bold;">商品清单</span><em style="font-size:12px;font-style: normal;float: right;background-color: #ea5313; color: #fff; padding: 0px 5px;">[展开]</em>
        </div>
        <div class="owl-carousel">
            <#list cartItemList  as map><#list map.items as cartItem><#assign total = total+cartItem.quantity>
                <div class="face"><a href="${base}/item/whichapp/${cartItem.goodsId}.html"><img src="${cartItem.pic}_437x374.png"></a>
                        ...
                        <dd>
                            <div class="order-goods" style="padding-bottom: 15px;">
                                ...
                                <div class="info"> <a href="${base}/item/whichapp/${cartItem.goodsId}.html">${cartItem.name}</a>
                                    ...
                                    <span style="color:#ea5313;font-size:15px;font-weight:bold;line-height:22px;">${(cartItem.price?number)?string('currency')}</span>
                                    <span style="font-style: normal;color: #ea5313;font-size:15px;line-height:22px;">共${cartItem.quantity}件</span>
        ...
        <div style="margin-left: 8px;">
                ...
                <div id="payment-ship" style="position: static;">
                    ...
                    <div id="delivery-time">
                        ...
                        <div class="middle">
                            ...
                            <div class="item" style="height: 28px;">
                                <div class="label">
                                    <input style="position: absolute; margin-left: 2px; margin-top: 1px;"  id="delivery-t3" name="delivery-t" payname="${deliveryTime.introduce}" payid="${deliveryTime.id}"  <#if deliveryTime_index==0>checked</#if>  value="${deliveryTime.id}" type="radio" onclick="$('#order_deliveryMethodName').val('${deliveryTime.introduce}');$('#order_deliveryMethodId').val('${deliveryTime.id}');">
			...
			<div class="step-title">
                ...
                <a href="javascript:void(0);" id="saveInvoiceTitleMinDiv" class="hide" style=" color: #ea5313;" onclick="save_Invoice()">保存发票信息</a></p>
			</div><!--step-title end-->
...
<div style=" background: #fff; text-align: right; padding: 15px 0; padding-right: 10px; border-bottom: 1px solid #d8d8d8;  overflow: hidden;width:96%;">
    <p style="font-size: 14px; margin-bottom: 5px;">共计 <span style="font-style: normal;color: #ea5313;font-weight: bold;">${total} </span>件商品</p>
    <p style="font-size: 14px; margin-bottom: 5px;">总商品金额：<em style="font-style: normal;color: #ea5313;font-weight: bold;"  id="warePriceId" v="${totalPrice?string('##0.00')}">￥${totalPrice?string('##0.00')}</em></p>
    <p style="font-size: 14px; margin-bottom: 5px;">运费： <em  style="font-style: normal;color: #ea5313;font-weight: bold;"  class="price" id="freightPriceId"> ￥${deliveryFee?string('##0.00')}</em></p>
    <p style="font-size: 14px; margin-bottom: 5px;">优惠合计： <em  style="font-style: normal;color: #ea5313;font-weight: bold;"  class="price" id="freightPriceId">￥${discount?string('##0.00')}</em></p>
</div>
...
<nav class="mui-bar mui-bar-tab">
    ...
    <#assign totalPrice = totalPrice + deliveryFee>
        <span style="margin:8px 10px; font-size: 16px; ">应付总额：<em  style="font-style: normal; font-size: 16px; font-weight: bold; color: #ea5313;"  id="sumPayPriceId" class="price"> ￥${(totalPrice?number-discount)?string('##0.00')}</em></span>
...
<script>
    ...
    function submit_Order(){
        ...
        jQuery.ajax({
            type: "post",
            url: fullpath + "/promotions/submit?whichclient=whichapp",
...
function isEmpty(value){
    ...
    function toReceiver(){
        ...
        var url=fullpath + "/member/receiver?whichclient=whichapp&to=promotions&o=${o}&paymentId="+paymentMethodId+"&deliveryId="+deliveryId+"&promotionsId=${promotionsId}&goodsId=${goodsId}&dealNum=${dealNum}";
        ...
        if(jQuery("#invoiceContent").val()=="no") {
            ...
            if(jQuery(".selected[type='invoiceType']").text()==""){
                ...
                jQuery.ajax({
                    type: "post",
                    url: fullpath + "/member/saveInvoiceSetting",
                    data: "invoiceTitle=" + jQuery(".selected[type='invoiceTitle']").attr("value") + "&id=" + jQuery(".selected[type='invoiceTitle']").attr("id") + "&invoiceContent=" + jQuery(".selected[type='invoiceContent']").text(),
                    dataType: "json",
                    success: function (data) {
                        if (data.success) {
                            popAlert(data.msg);
                            var info = jQuery(".selected[type='invoiceType']").text() + "　　" + jQuery(".selected[type='invoiceTitle']").attr("value") + "　　" + jQuery(".selected[type='invoiceContent']").text();
