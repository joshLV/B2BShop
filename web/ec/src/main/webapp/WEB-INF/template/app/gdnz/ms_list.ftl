...
<div class="mui-content" id="content" style="padding-bottom:58px; padding-top:50px;">
        <div class="ms_list">
            <ul>
                ...
                <li id="${p.pgId}" >
                    <div class="face">
                        <a href="${base}/item/whichapp/${p.goodsId}.html?mt=m&promotionsId=${p.id}"><img src="<#if p.url??>${imageUrl}${p.url}@205w_224h.png</#if>"></a>
                    </div>
                    <div class="info">
                        <div style="font-size: 14px;color: red">距离活动<#if p.status == 'ing'>结束<#elseif p.status == 'before'>开始</#if></div>
                        <div class="pms_time_interval" style="font-size: 14px;color: red"></div>
