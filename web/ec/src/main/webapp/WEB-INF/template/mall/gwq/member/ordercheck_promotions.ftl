...
<style>
    ...
    #checkout {/*width:988px;*/ border:1px solid #ddd; margin-bottom: 20px; margin-top: 15px; background: #ffffff;}
    ...
    .checkout-buttons .checkout-submit{background-color:#C30000;position:relative;line-height:36px;overflow:hidden;color:#fff;font-weight:bold;font-size:16px;border-radius: 3px;}
    ...
    .checkout-buttons .checkout-submit:hover{background-color:#C30000;}
    ...
    .checkout-buttons .checkout-submit-disabled b{position:absolute;left:0;top:0;width:135px;height:36px;background:#C30000;cursor:not-allowed; border-radius: 3px;}
    ...
    #order-cart .review-thead {width:100%;  border-top:2px solid #C30000;  border-right:1px solid #e8e8e8; border-left:1px solid #e8e8e8; }
    ...
    #order-cart .p-goods {width:426px;}
    #order-cart .p-goods {overflow:hidden; zoom:1;}
    #order-cart .p-img {width:70px; height:70px; margin-right:5px; border:1px solid #ddd; padding:0; float:left; overflow:hidden;}
    ...
    #checkout .btn-submit {height:30px; margin-right:10px; padding-left:20px; float:left; line-height:30px; font-size:14px; font-weight:700;background:#C30000; border-radius: 3px;}
    #checkout .btn-submit span {height:30px; padding-right:20px; float:left; cursor:pointer;background:#C30000;border-radius: 3px;}
    ...
    .btn{color:#ffffff; background: #e54044; padding: 0 10px 0 10px; float: left; height: 30px; line-height: 30px; margin-left: 5px; margin-top: 7px;}
    .btn:hover {color: #ffffff;}
    .items .radioitem{padding:5px 10px; margin: 0 10px; height: 18px; line-height: 18px; margin-top: 3px;}
    .items .selected{border: #f43838 solid 1px;  height: 18px; line-height: 18px;}
    ...
    #payment-ship .item-selected {background-color:#FFF6F6;}
    ...
    .Validform_right{
        color:#71b83d;
        padding-left:20px;
        background:url(${base}/static/mall/gwq/img/right.png) no-repeat left center;
    }
    .Validform_wrong{
        color:red;
        padding-left:20px;
        white-space:nowrap;
        background:url(${base}/static/mall/gwq/img/error.png) no-repeat left center;
    }
    .Validform_loading{
        padding-left:20px;
        background:url(${base}/static/mall/gwq/img/onLoad.gif) no-repeat left center;
    }
    .Validform_error{
    ...
    <#include "mall/gwq/header_new2.ftl"/>
    <#import "mall/gwq/header_new2.ftl" as header_new2>
    <@header_new2.menu id="2"/>
...
<div id="checkout" class="content">
    ...
    <div id="wizard" class="checkout-steps">
        <form id="orderCheckInfo">
            ...
            <input type="hidden" name="promotionsId" id="promotionsId" value="${promotionsId}"/>
            <input type="hidden" name="goodsId" id="goodsId" value="${goodsId}"/>
            <input type="hidden" name="dealNum" id="dealNum" value="${dealNum}"/>
            <input type="hidden" name="resultNum" id="resultNum" value="${resultNum}"/>
        </form>
                            ...
                            <ul class="adr_list" id="consignee-list">
                                <#list receiverList as receiver>
                                    ...
                                    <div class="edit_box clearfix">
                                        ...
                                        <#else>
                                            <a id="addrSet" class="btn_set_common" onClick="setDefault('${receiver.id}')" href="javascript:void(0)">设为默认地址</a></div>
                                        </#if>
        ...
        <div class="step-title">
            ...
            <strong>支付及送货时间</strong>
				<span class="step-action" id="payment-ship_edit_action">
					...
					<a href="javascript:void(0);" id="savePayAndShipTitleMinDiv" class="hide" style="color:#005EA7;" onclick="save_PayAndShip()">保存支付及送货时间</a>
				</span>
        </div><!--step-title end-->
                ...
                <div class="hide" id="payAndShip-edit-div">
                    <div id="payment-ship" style="position: static;">
                        ...
                        <div class="form-btn group" >
                            <a href="javascript:void(0);" class="btn-submit" onclick="save_PayAndShip()"><span>保存支付及送货时间</span></a>
                        </div>
    ...
    <div id="step-3" class="step step-complete"><a name="invoiceFocus"></a>
        <div class="step-title">
            ...
            <span style="padding-left: 50px;font-weight: bold;color: red;">注：跨境商品不能开具发票</span>
        </div><!--step-title end-->
            ...
            <div id="part-invoice" class="sbox-wrap">
                <div class="sbox">
                    ...
                    <div class="form hide" id="invoice-edit-div">
                            ...
                            <ul class="items">
                                ...
                                <#list memberInvoicesList as invoice>
                                    <li class="radioitem invoiceTitel <#if invoice.invoiceTitle==memberInvoice.invoiceTitle>selected</#if>" type="invoiceTitle" id="${invoice.id}" value="${invoice.invoiceTitle}">
                                        ...
                                        <b onclick="deleteInvoice('${invoice.id}','${invoice.invoiceTitle}','${invoice_index}');">X</b>
                                    </li>
                                </#list>
                            </ul>
                        </div>
                        <div>
                            <ul class="items">
                                <li class="hide addnew">
                                    <label for="title"><strong><span id="action">新增</span>发票抬头：</strong></label>
                                    <input type="text" name="title" id="title" style="margin-top: 7px; width: 300px;height: 28px;border: 1px solid #E8E8E8;padding-left: 5px;"/>
                                    <input type="hidden" name="id" id="id" value=""/>
                                </li>
                                <li>
                                    &nbsp;&nbsp;<a href="javascript:;" class="closeaddnew btn" onclick="addInvoiceTitle();">新增</a>
                                    &nbsp;<a href="javascript:;" class="hide addnew btn" onclick="saveInvoiceTitle();">保存</a>
                                    &nbsp;<a href="javascript:;" class="hide addnew btn" onclick="closeaddInvoiceTitle();">关闭</a>
                                </li>
                            </ul>
                        </div>
                        <div>
                        ...
                        <div class="form-btn group" style="margin-top: 10px;">
                            <a href="javascript:void(0);" id="saveInvoiceTitleMinDiv" class="btn-submit" onclick="save_Invoice()"><span>保存发票信息</span></a>
                        </div>
                    </div><!--payAndShip-edit-div end-->
                </div>
                        ...
                        <div class="order-review"><!--商品清单展示-->
    							...
    							<span id="span-skulist">
									...
									<div class="review-body" >
                                        ...
                                        <#-- 添加商家名称 TianYu 2015-8-19  -->
                                            <div class="sj-name" style="display:none;">
                                                        ...
                                                        <table class="order-table">
                                                            ...
                                                            <tr >
                                                                <td class="fore1" style=" padding-bottom: 10px;">
                                                                    <div class="p-goods">
                                                                        <div class="p-img">
                                                                            <a href="${base}/item/${cartItem.goodsId}.html" target="_blank"><img height="70" width="70" alt="" src="${cartItem.pic}@70w_70h.png"></a>
                                                                        </div>
                                                                        <div class="p-detail">
                                                                            <div class="p-name">
                                                                ...
                                                                <td class="p-promotion">
                                                                    <ul>
                                                                        ...
                                                                        <li>满减优惠：${(cartItem.mjDecreaseAmount?number)?string('currency')}</li>
                                                                        </#if>
                                                                        <li>等级优惠：${((cartItem.price?number * memberRankScale?number)*cartItem.quantity?number)?string('currency')}</li>
                                                                    </ul>
                                                                </td>
                            ...
                            <div class="order-summary">
                                <div class="statistic fr">
                                    <div class="list">
                                        <span><em id="span-skuNum">${total}</em> 件商品，总商品金额：</span><em class="price" id="warePriceId" v="${totalPrice?string('##0.00')}">${totalPrice?string('currency')}</em>
                                    </div>
                                    <div class="list">
                                        <span>优惠合计：</span>
            ...
            <div id="checkout-floatbar" class="checkout-buttons group">
                <div class="sticky-wrap">
                    <div class="inner">
                        ...
                        <span class="total">应付总额：<strong id="payPriceId">${(totalPrice + deliveryFee - discount)?string('##0.00')}</strong>元 </span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="pop_win aptab_center_address">
            <ul class="newaddress">
                ...
                <li>
                    ...
                    <input type="text" name="consigneeParam.address" style="width: 398px;" id="consignee_address" maxlength="50"  class="inputxt" >
                </li>
                <li>
                    ...
                    <input type="text" value=""  id="consignee_phone" name="consigneeParam.phone" class="inputxt tel" style="width: 110px;" ignore="ignore"  >
...
//        $("#tips_"+index).show();
    ...
    function save_PayAndShip() {
        ...
        $("#payAndShip-edit-div").hide();//delivery-t
    ...
    function submit_Order(){
        ...
        if(parseInt(limitNum)!=0) {
            var beBuy = parseInt("${dealNum}") + parseInt(quantity);
            if(parseInt(${goods.stock})<parseInt(startNum)) {
                layer.alert("亲, 小的正在积极备货, 客官请稍等片刻...", {icon:5});
                window.location.href = "${base}/item/${goodsId}.html";
                return;
            }

            if(parseInt("${dealNum}")>parseInt(${goods.stock}) && parseInt("${dealNum}")>parseInt(startNum)) {
                layer.alert("亲, 小的存货不足, 客官下次再来看看吧", {icon:5});
                window.location.href = "${base}/item/${goodsId}.html";
                return;
            }

            if(parseInt(beBuy)>parseInt(limitNum)) {
                layer.alert("亲, 您还可以购买"+ (parseInt(limitNum)-parseInt(quantity)) +"件商品", {icon:5});
                window.location.href = "${base}/item/${goodsId}.html";
                return;
            }
            ...
            if(parseInt(beBuy)>parseInt(limitNum)) {
                if((parseInt(limitNum)-parseInt(quantity))<=0) {
                    layer.alert("亲, 您的购买资格已使用完了, 下次再接再厉", {icon:5});
                    window.location.href = "${base}/item/${goodsId}.html";
                    return;
                } else {
                    layer.alert("亲, 您还可以购买"+ (parseInt(limitNum)-parseInt(quantity)) +"件商品", {icon:5});
