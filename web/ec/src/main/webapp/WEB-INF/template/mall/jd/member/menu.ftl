...
<style>
/*leftMenu*/
#leftMenu .mt{height:32px;padding:0 6px;background-color:#CCCCCC;border-top:2px solid #e4393c;line-height:32px;color:#fff;}
...
#leftMenu .mc{border:1px solid #CCCCCC; border-top: none;}
...
#leftMenu dl dt b{display:block;position:absolute;top:12px;left:173px;width:9px;height:5px;background:url(${base}/static/mall/img/left.png) -64px 0;font-size:0;line-height:0;overflow:hidden}
...
<link href="${base}/static/mall/gwq/css/fix.css" rel="stylesheet" type="text/css" media="all"/>
		...
		<dl tag="${plMenu.orderNo}">
			...
			<dd>
				<#list childList as clMenu>
					<#if plMenu.id==clMenu.parentId>
					<div class="item">
                        <#if clMenu.name == select>
						    <strong><a href="${base}/${clMenu.link}" tag="${clMenu.orderNo}">${clMenu.name}</a><#if clMenu.isnew==1><img width="24" height="11" src="${base}/static/mall/img/new-ico.png"></#if></strong>
                        <#else>
						    <a href="${base}/${clMenu.link}" tag="${clMenu.orderNo}">${clMenu.name}</a><#if clMenu.isnew==1><img width="24" height="11" src="${base}/static/mall/img/new-ico.png"></#if>
                        </#if>
					</div>
					</#if>
				</#list>
