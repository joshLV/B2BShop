...
<style>
    ...
    /*jqzoom*/
    .jqzoom{ width:420px; height:360px; /*border:1px solid #e6e6e6;*/ position:relative;overflow:hidden; background: url("${base}/static/mall/jp/img/default_logo.jpg") no-repeat center;}
    ...
    .lh li{width:190px; text-align: center; padding: 10px 0; margin-left: 8px;}
    .lh li p{font-size: 14px; font-weight: 600; color: #ff0000;}
    #related-sorts li{padding:0; margin: 0;}
</style>
            ...
            <div <#--class="clearfix"-->style=" width: 618px;border-bottom: 1px dotted #e8e8e8;">
                ...
                <#--Desc:商品编号没有什么作用，去掉 Auth:zhangqiang Time:2015-11-23 16:34:45 End-->
                    ...
                    <li id="summary-service">
                        ...
                        <div class="dd">由 ${(shop.name?exists)?string("<a href='${base}/shop/${shop.urlAddress}?pageSize=20'>${shop.name}</a>","本商城")} 发货并提供售后服务。
                            <#if shop.qqKeFu??>
    ...
    function getMaxSalesCountGoods(obj){
        ...
        if(!isEmpty(id)){
            $.ajax({
                type: 'post',
                async: false,
                data: "ajaxCatelogId="+id+"&gid=${goods.id}&shopId=${goods.shopId}&r="+ new Date(),
                    ...
                    if( data.status == 'success' ){
                        $.each(data.list, function(i, item) {
                            if(item !="" && item !=undefined){
                                ...
                                allHtml +="<a href='"+$("#baseId").val()+"/item/"+item.id+".html' target='_blank' title='"+item.fullName+"'><img alt='"+item.fullName+"' src='"+item.pic+"@190w_140h.png' /></a>";
