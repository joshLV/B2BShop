...
<style>
    ...
    .news-list li{overflow:hidden;height:24px;padding:0 0 0 5px;line-height:24px;/*background:url(${base}/static/mall/jp/img/dian.gif) 0 11px no-repeat;*/}
    ...
    .i_uname{ background:#fff url(${base}/static/mall/jp/img/i_uname.gif) 7px 6px no-repeat;}
    .i_upass{ background:#fff url(${base}/static/mall/jp/img/i_upass.gif) 7px 6px no-repeat;}
    ...
    .sy_sub{ width:85px; height:28px; background:url(${base}/static/mall/jp/img/dl.gif) no-repeat; cursor:pointer; border:none;}
...
/*zyf  3-24*/
    ...
    .slider-prev{ position: absolute; left: 16px; width: 25px; height: 64px; top:80px; background:url(${base}/static/mall/jp/img/arr.png) left top no-repeat; text-indent: -9999px;z-index: 100;}
    .slider-prev:hover{ background:url(${base}/static/mall/jp/img/arr_on.png) left top no-repeat;
        -webkit-transition: all .4s ease-out;
        -moz-transition: all .4s ease-out;
        -ms-transition: all .4s ease-out;
        -o-transition: all .4s ease-out;
        transition: all .4s ease-out;}
    .slider-next{ position: absolute; right: 16px; width: 25px; height: 64px; top:80px; background:url(${base}/static/mall/jp/img/arr.png) -35px top no-repeat; text-indent: -9999px; z-index: 100;}
    .slider-next:hover{background:url(${base}/static/mall/jp/img/arr_on.png) -35px top no-repeat;
        -webkit-transition: all .4s ease-out;
        -moz-transition: all .4s ease-out;
        -ms-transition: all .4s ease-out;
        -o-transition: all .4s ease-out;
    ...
    <div class="w11210">
            ...
            <div class="slide-right">
                ...
                <div class="sy_login" id="login">
                    <h1>会员登录</h1>
                    <div class="user_img">
                        <img src="${base}/static/mall/jp/img/user_img.jpg">
                        <p>hi! 你好<br>欢迎来到 jp516.com</p>
                    </div>
                ...
                <div class="sy_login" id="loginAfter" style="margin-bottom: 0;">
                    <div class="user_img" style="height: 80px;">
                      <img src="${base}/static/mall/jp/img/user_img.jpg" style="margin-top:10px;">
            ...
            <#--  <div style="width: 238px; height:153px; float: left; margin-bottom:5px;">
                  <a href="#">
                      <img src="${base}/static/mall/jp/img/p5.jpg" width="238" height="153">
        ...
        <div class="hot-region" id="hot-region">
            <div class="hot-region-hd">
                <ul id="hot-region-nav" class="hot-region-nav clearfix">
                    <#list tagList as tag>
                        <li class="<#if tag_index==0>cur<#elseif tag_index==3>hide</#if>" tagSign="${tag["sign"]}">${tag["name"]}</li><!--${tag["sign"]}-->
                    </#list>
                    <li class="hide" tagSign="todayCommend">今日爆款</li>
                </ul>
            </div>
            <div class="hot-region-bd" id="tag_goods_div">
                <#list tagGoodsList as tagGoodsMap>
                <div style="width: 100%;position: relative;">
                    <input type="hidden" id="${tagGoodsMap["sign"]}_pn" value="1">
                    <input type="hidden" id="${tagGoodsMap["sign"]}_pc" value="1">
                    <a href="javascript:" class="slider-prev" onclick="ajaxGetIndexGoodsByTag('${tagGoodsMap["sign"]}', -1)">左</a>
                    <a href="javascript:" class="slider-next" onclick="ajaxGetIndexGoodsByTag('${tagGoodsMap["sign"]}', 1)">右</a>
                    <ul class="products" style="display: <#if tagGoodsMap_index==0>block<#else>none</#if>;" id="${tagGoodsMap["sign"]}_ul">
                    </ul>
                </div>
                </#list>
                <div style="width: 100%;position: relative;">
                    <input type="hidden" id="todayCommend_pn" value="1">
                    <input type="hidden" id="todayCommend_pc" value="1">
                    <a href="javascript:" class="slider-prev" onclick="ajaxGetIndexGoodsByTag('todayCommend', -1)">左</a>
                    <a href="javascript:" class="slider-next" onclick="ajaxGetIndexGoodsByTag('todayCommend', 1)">右</a>
                    <ul class="products" style="display: none；" id="todayCommend_ul">
                    </ul>
                </div>
            </div>
        </div><!--hot-region end-->
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        ajaxGetIndexGoodsByTag('todayCommend',0);

//        setInterval("tagTimer(1)", 3000);

        $("#tag_goods_div").on("mouseover",function(){
            mouseSelct = false;
            $(this).trigger("click");
    ...
    function tagTimer(next){
        if(mouseSelct){
            var tagSign = '';
            $("#hot-region-nav li").each(function(){
                if($(this).attr("class")=='cur'){
                    tagSign = $(this).attr("tagSign");
                    return;
                }
            });
            ajaxGetIndexGoodsByTag(tagSign, next);
        }
    }
            ...
            success: function(data) {
                if (data.status == 'success') {
                    ...
                    $.each(data.list,function(k,v){
                        html += '<li class="fore'+ (k+pageNum*6) +'">';
                        html += '<p class="pic">';
                        html += '<a href="${base}/item/'+ v.id+'.html" target="_blank"><img alt="'+v.fullName+'" src="'+ v.pic+'@160w_160h.png"></a>';
                        html += '</p>';
                        html += '<p class="name"><a href="${base}/item/'+ v.id+'.html" target="_blank" title="'+v.fullName+'">'+v.name+'</a></p>';
                        html += '<p class="price">';
                        html += '商城价:';
                        html += '<strong>';
                        if(v.price == 0){
