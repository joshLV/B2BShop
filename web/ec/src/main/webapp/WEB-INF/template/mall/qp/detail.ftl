...
<head>
    ...
    <script src="${base}/static/js/cloudzoom.js" type="text/javascript"></script>
            ...
            <div class="clearfix">
                ...
                <ul id="choose">
                    <#list spInfoMap["goodsSpList"] as sp>
                        <li id="choose-${sp.id}">
                            <div class="dt" id="${sp.id}_sp">选择：</div>
                            <#list spInfoMap["goodsSpItemList"] as item>
                                <#if item.specificationId == sp.id>
                                    <div style="float: left;">
                                        <div class="item" style="min-width: 35px;text-align: center;margin-top: -2px">
                                            <#if sp.type == 1>
                                                <a href="${base}/item/${item.goodsId}.html" id="${item.id}_item" title="${item.name}">
                                                ${item.name}<img src="${item.pic}" style="margin-top: 1px;padding-left: 1px;padding-right: 3px" width="16" height="16">
                                                </a>
                                            <#else>
                                                <a href="${base}/item/${item.goodsId}.html" id="${item.id}_item" title="${item.name}">${item.name}</a>
                                            </#if>
                                        </div>
                                    </div>
                                </#if>
